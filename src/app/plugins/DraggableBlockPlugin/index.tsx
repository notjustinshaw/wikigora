...
const DRAGGABLE_BLOCK_MENU_CLASSNAME =
	"rounded-md p-1 cursor-grab opacity-0 absolute top-0 left-0 will-change-transform";
...
function getBlockElement(
	...
	editor.getEditorState().read(() => {
		...
		// cannot drag title anywhere or drag items above title
...
function useDraggableBlockMenu(
	...
	return createPortal(
		<>
			<Image
				className={cx(
					"hidden lg:flex rounded-md py-1 absolute top-0 left-0 will-change-transform w-6 h-6",
					"cursor-grab active:cursor-grabbing hover:bg-neutral-100 select-none"
				)}
				src={"/drag.svg"}
				alt='Draggable Block Menu'
				width={32}
				height={32}
				ref={menuRef}
				draggable={true}
			...
			/>
			<div
				className='pointer-events-none bg-sky-400 h-1 absolute top-0 left-0 opacity-0 will-change-transform'
				ref={targetLineRef}
			/>
		</>,
		anchorElem
